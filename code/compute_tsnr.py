...
def main() -> int:
    ...
    data=np.asanyarray(img.dataobj,dtype=np.float32); mask=np.asanyarray(mask_img.dataobj)>0
    mean=np.mean(data,axis=3); sd=np.std(data,axis=3,ddof=1)
    tsnr=np.zeros(mean.shape,dtype=np.float32); valid=mask & np.isfinite(mean) & np.isfinite(sd) & (sd>0)
    ...
    out={
      ...
      'input':str(a.input.resolve()),'mask':str(a.mask.resolve()),'n_volumes':img.shape[3],
      'tr_seconds':float(img.header.get_zooms()[3]),'mask_voxels':int(mask.sum()),
      'valid_voxels':int(values.size),'coverage_pct':float(100*values.size/mask.sum()),
